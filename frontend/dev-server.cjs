...
app.post('/api/clubs', async (req, res) => {
  try {
    ...
    if (err.message === 'SLUG_EXISTS') return res.status(409).json({ error: 'Slug already taken' });
...
app.post('/api/clubs/:clubId/members/import', async (req, res) => {
  try {
    ...
    if (!ctx) return res.status(401).json({ error: 'Authentication required' });
    if (!['GESTOR', 'ADMIN'].includes(ctx.role))
      return res.status(403).json({ error: 'Only GESTOR/ADMIN can bulk import' });
...
app.post('/api/clubs/:clubId/athletes', async (req, res) => {
  try {
    ...
    if (!ctx) return res.status(401).json({ error: 'Authentication required' });
    if (!['GESTOR', 'ADMIN'].includes(ctx.role))
      return res.status(403).json({ error: 'Apenas GESTOR/ADMIN pode cadastrar atletas' });
    ...
    if (!VALID_ROLES.includes(role)) return res.status(400).json({ error: 'Papel inválido' });
    ...
    if (role === 'COACH') {
      if (!email || !email.trim())
        return res.status(400).json({ error: 'E-mail é obrigatório para técnicos.' });
      ...
      if (existingMembership)
        return res.status(409).json({ error: 'Técnico já vinculado a este clube.' });
    ...
    // ─── Fluxo ATHLETE: cria User (login via CPF) + AthleteProfile + ClubMembership ───
    let userId = null;
    const loginIdentifier = cleanCpf || (email ? email.trim().toLowerCase() : null);
...
app.patch('/api/clubs/:clubId/members/:membershipId/profile', async (req, res) => {
  try {
    const ctx = await extractCtx(req);
    if (!ctx) return res.status(401).json({ error: 'Authentication required' });
    const { clubId, membershipId } = req.params;
    if (ctx.role !== 'ADMIN' && (ctx.role !== 'GESTOR' || ctx.clubId !== clubId))
      return res.status(403).json({
    ...
    console.error('[PATCH /api/clubs/:clubId/members/:membershipId/profile]', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
app.post('/api/admin/clubs', async (req, res) => {
  try {
    ...
    if (!gestorPassword || gestorPassword.length < 6)
      return res.status(400).json({ error: 'Senha do gestor deve ter ao menos 6 caracteres.' });
    ...
    if (slugExists) return res.status(409).json({ error: `Slug "${clubSlug}" já está em uso.` });
    ...
    if (err.code === 'P2002')
      return res.status(409).json({ error: 'E-mail do gestor ou slug já cadastrado.' });
...
app.patch('/api/matches/:id/state', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(`[PATCH /${req.params.id}/state] Erro ao parsear matchState:`, e);
    }
    ...
    if (!currentMatch) return res.status(404).json({ error: 'Partida não encontrada' });
    ...
    console.log(`[PATCH /${req.params.id}/state] Status: ${currentMatch.status} → ${status}`);
...
app.get('/api/admin/matches/all', async (req, res) => {
  try {
    const ctx = await extractCtx(req);
    if (!ctx) return res.status(401).json({ error: 'Authentication required' });
    if (ctx.role !== 'ADMIN') return res.status(403).json({ error: 'Acesso negado' });
