// frontend/api/_handlers/_tournaments.ts
...
function getTournamentId(url: URL): string | null {
    ...
    if (req.method === 'PATCH') {
      try {
        ...
        if (existing && ['FINISHED', 'CANCELLED'].includes(existing.status))
          return sendJson(res, 409, { error: 'Tournament already finished or cancelled' });

        const { name, description, startDate, endDate, status, maxPlayers, rules, courtType } =
          (req.body ?? {}) as {
            name?: string;
            description?: string;
            startDate?: string;
        ...
        if (maxPlayers !== undefined)
          updateData.maxPlayers = maxPlayers ? parseInt(String(maxPlayers)) : null;
      ...
      if (action === 'add-entry') {
        try {
          if (ctx.role === 'SPECTATOR')
            return sendJson(res, 403, { error: 'Spectators cannot add entries' });

          const { athleteId, categoryId, seed } = (req.body ?? {}) as {
            athleteId?: string;
            categoryId?: string;
            seed?: number | string;
          };
          ...
          if (!['DRAFT', 'REGISTRATION'].includes(tournament.status))
            return sendJson(res, 400, { error: 'Tournament is not accepting registrations' });
          if (tournament.maxPlayers && tournament._count.entries >= tournament.maxPlayers)
          ...
          if (!tournament.isOpenToExternalAthletes && tournament.clubId) {
            ...
            if (athlete && athlete.clubId !== tournament.clubId)
              return sendJson(res, 403, { error: 'Tournament does not accept external athletes' });
          }
          ...
          const entry = await prisma.tournamentEntry.create({
            data: {
              ...
              seed: seed ? parseInt(String(seed)) : null,
              status: 'REGISTERED',
            },
            include: {
              athlete: {
          ...
          if ((err as { code?: string }).code === 'P2002')
            return sendJson(res, 409, { error: 'Athlete already registered in this category' });
  ...
  if (req.method === 'POST') {
    try {
      if (!['ADMIN', 'GESTOR', 'COACH'].includes(ctx.role ?? ''))
        return sendJson(res, 403, { error: 'Insufficient permissions to create tournament' });
      ...
      if (!ctx.clubId)
        return sendJson(res, 400, {
          error: 'Associação a um clube é necessária para criar torneio.',
        });
      ...
      const tournament = await prisma.tournament.create({
        data: {
          ...
          maxPlayers: maxPlayers ? parseInt(String(maxPlayers)) : null,
