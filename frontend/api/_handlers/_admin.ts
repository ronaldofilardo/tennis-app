// frontend/api/_handlers/_admin.ts
import type { ServerResponse } from 'node:http';
import type { MatchStatus, PlanType } from '@prisma/client';
...
function getSection(url: URL): string | null {
...
export default async function handler(req: ApiRequest, res: ServerResponse): Promise<void> {
  ...
  if (section === 'clubs' && req.method === 'POST') {
    ...
    if (!gestorName || !gestorName.trim()) return sendJson(res, 400, { error: 'Nome do gestor é obrigatório.' });
    if (!gestorEmail || !gestorEmail.trim()) return sendJson(res, 400, { error: 'E-mail do gestor é obrigatório.' });
    if (!gestorPassword || gestorPassword.length < 6)
      return sendJson(res, 400, { error: 'Senha do gestor deve ter ao menos 6 caracteres.' });
    ...
    try {
      const slugExists = await prisma.club.findUnique({ where: { slug: clubSlug } });
      if (slugExists) return sendJson(res, 409, { error: `Slug "${clubSlug}" já está em uso.` });
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (!gestor) {
          gestor = await tx.user.create({
            data: { email: cleanEmail, name: gestorName.trim(), passwordHash, isActive: true },
          });
        }

        const club = await tx.club.create({
          data: { name: name.trim(), slug: clubSlug, planType: planType as PlanType },
        });
      ...
      if ((err as { code?: string }).code === 'P2002')
        return sendJson(res, 409, { error: 'E-mail do gestor ou slug já cadastrado.' });
  ...
  if (section === 'matches') {
    ...
    if (subSection === 'all') {
      ...
      try {
        const mlimit = Math.min(parseInt(url.searchParams.get('limit') || '20', 10), 100);
        const moffset = parseInt(url.searchParams.get('offset') || '0', 10);
        const statusFilter = url.searchParams.get('status') || null;
        const where = statusFilter ? { status: statusFilter as MatchStatus } : {};
        ...
        return sendJson(res, 200, { matches: formatted, total, limit: mlimit, offset: moffset });
      } catch (err) {
  ...
  if (section === 'athletes') {
    ...
    if (subSection === 'sync-passwords') {
      if ((req.method as string) !== 'POST') return methodNotAllowed(res, ['POST']);
      try {
            ...
            profiles.map(async (profile) => {
              try {
                ...
                const senha = derivarSenha(
                  profile.birthDate ? profile.birthDate.toISOString().split('T')[0] : null,
                  cleanCpf,
                );
