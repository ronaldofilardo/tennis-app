// frontend/api/_handlers/_athletes.ts
...
function getAthleteId(url: URL): string | null {
  ...
  if (athleteId) {
    ...
    if (req.method === 'PATCH') {
      try {
        ...
        if (ranking !== undefined) updateData.ranking = ranking ? parseInt(String(ranking)) : null;
