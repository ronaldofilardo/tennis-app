...
export default async function handler(req, res) {
  ...
  if (athleteId) {
    if (req.method === "GET") {
      try {
        ...
        const response = isOwnClub || isSelf
          ? athlete
          : { id: athlete.id, name: athlete.name, nickname: athlete.nickname, clubId: athlete.clubId, category: athlete.category, gender: athlete.gender, ranking: athlete.ranking };
    ...
    if (req.method === "PATCH") {
      try {
        ...
        const isOwnClub = ctx.clubId && athlete.clubId === ctx.clubId;
        const isSelf = athlete.userId && athlete.userId === ctx.userId;
        if (!isOwnClub && !isSelf) return sendJson(res, 403, { error: "Cannot edit athlete from another club" });
        ...
        const updated = await prisma.athleteProfile.update({ where: { id: athleteId }, data: updateData });
  ...
  if (req.method === "GET") {
    try {
      ...
      const where = {
        isPublic: true,
        ...(sanitized && { name: { contains: sanitized, mode: "insensitive" } }),
        ...(filterClubId && { clubId: filterClubId }),
      };
      const athletes = await prisma.athleteProfile.findMany({
        where, take: limit, orderBy: { name: "asc" },
        select: { id: true, name: true, nickname: true, clubId: true, category: true, gender: true, ranking: true, birthDate: true },
      });
  ...
  if (req.method === "POST") {
    try {
      const { name, nickname, birthDate, phone, category, gender, ranking, isPublic = true } = req.body || {};
