...
import EditMemberModal, {
  type EditableMember,
} from "../src/components/EditMemberModal";
...
describe("EditMemberModal", () => {
  ...
  describe("Visibilidade", () => {
    ...
    it("não renderiza nada quando member=null", () => {
      render(
        <EditMemberModal {...defaultProps} isOpen={true} member={null} />,
      );
      expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    });
  });
  ...
  describe("Render — Atleta (ATHLETE)", () => {
    ...
    it("preenche campos do atleta após carregar perfil", async () => {
      ...
      expect(
        (screen.getByLabelText(/apelido/i) as HTMLInputElement).value,
      ).toBe("Carlão");
      expect(
  ...
  describe("Submit — Técnico (COACH)", () => {
    ...
    it("omite email do payload quando campo e-mail está vazio", async () => {
      ...
      await waitFor(() => {
        const [, payload] = mockHttpPatch.mock.calls[0] as [
          string,
