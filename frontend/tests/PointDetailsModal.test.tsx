...
describe('PointDetailsModal', () => {
  ...
  describe('Renderização básica', () => {
    it('não renderiza quando isOpen é false', () => {
      ...
      expect(document.querySelector('.point-details-modal')).not.toBeInTheDocument();
    });

    it('renderiza o modal quando isOpen é true', () => {
      render(<PointDetailsModal {...defaultProps} />);
      expect(document.querySelector('.point-details-modal')).toBeInTheDocument();
    });
    ...
    it('exibe nome de PLAYER_2 como vencedor quando corresponde', () => {
      render(
        <PointDetailsModal {...defaultProps} playerWinner="PLAYER_2" currentServer="PLAYER_1" />,
      );
  ...
  describe('fontScale → variável CSS --sb-scale', () => {
    it('aplica --sb-scale=1 por padrão (sem prop fontScale)', () => {
      render(<PointDetailsModal {...defaultProps} />);
      const modal = document.querySelector('.point-details-modal') as HTMLElement;
    ...
    it('atualiza --sb-scale ao re-render com novo fontScale', () => {
      const { rerender } = render(<PointDetailsModal {...defaultProps} fontScale={1} />);
      const modal = document.querySelector('.point-details-modal') as HTMLElement;
      expect(modal).toHaveStyle({ '--sb-scale': '1' });
