...
// ── Mock httpClient ────────────────────────────────────────────────────────
const { mockGet, mockHttpClientObj } = vi.hoisted(() => {
  const mockGet = vi.fn();
  const mockHttpClientObj = {
    get: mockGet,
    patch: vi.fn(),
    post: vi.fn(),
    put: vi.fn(),
  ...
  return { mockGet, mockHttpClientObj };
});
vi.mock('../src/config/httpClient', () => ({
  default: mockHttpClientObj,
  httpClient: mockHttpClientObj,
}));
...
describe('AthleteSearchInput — busca básica', () => {
  ...
  it('não chama a API para queries com menos de 2 caracteres', async () => {
    ...
    // garante que nenhum update assíncrono do debounce foi disparado
    await waitFor(() => expect(mockGet).not.toHaveBeenCalled(), { timeout: 500 });
  });
...
describe('AthleteSearchInput — prop excludeUserId', () => {
  ...
  it('inclui excludeUserId na URL quando a prop é fornecida', async () => {
    render(<AthleteSearchInput onSelect={vi.fn()} excludeUserId="user-xyz-123" />);
    fireEvent.change(screen.getByRole('combobox'), {
      target: { value: 'Bruno' },
    });
    await waitFor(() => expect(mockGet).toHaveBeenCalled(), {
  ...
  it('mantém o encodamento correto para IDs com caracteres especiais', async () => {
    render(<AthleteSearchInput onSelect={vi.fn()} excludeUserId="user@id+test" />);
    fireEvent.change(screen.getByRole('combobox'), {
      target: { value: 'Ana' },
    });
    await waitFor(() => expect(mockGet).toHaveBeenCalled(), {
