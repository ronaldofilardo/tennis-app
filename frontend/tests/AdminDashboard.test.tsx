...
describe('AdminDashboard', () => {
  ...
  describe('KPIs administrativos/financeiros', () => {
    ...
    it('exibe novos usuários do mês', async () => {
      render(<AdminDashboard />);
      expect(await screen.findByText(/Novos usu\u00e1rios/i)).toBeInTheDocument();
  ...
  describe('Seções removidas (não pertencem ao Admin)', () => {
    it('NÃO exibe "Partidas por Status"', async () => {
      ...
      expect(screen.queryByText(/Partidas por Status/i)).not.toBeInTheDocument();
    });

    it('NÃO exibe "Torneios por Status"', async () => {
      render(<AdminDashboard />);
      await waitFor(() => expect(mockHttpGet).toHaveBeenCalled());
      expect(screen.queryByText(/Torneios por Status/i)).not.toBeInTheDocument();
    });
    ...
    it('NÃO exibe "Total Partidas" nos KPIs', async () => {
      ...
      expect(screen.queryByText(/^Total de Partidas$/i)).not.toBeInTheDocument();
  ...
  describe('Tabs', () => {
    ...
    it('busca clubes ao clicar na aba Clubes', async () => {
      ...
      await waitFor(() => {
        expect(mockHttpGet).toHaveBeenCalledWith(expect.stringContaining('/admin/clubs'));
      });
    });
    ...
    it('busca usuários ao clicar na aba Usuários', async () => {
      ...
      await waitFor(() => {
        expect(mockHttpGet).toHaveBeenCalledWith(expect.stringContaining('/admin/users'));
      });
    });
  });
  ...
  describe('Tabela de Clubes - colunas corretas', () => {
    it('tabela de clubes NÃO tem colunas de Partidas ou Torneios', async () => {
      ...
      expect(screen.queryByRole('columnheader', { name: /Partidas/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('columnheader', { name: /Torneios/i })).not.toBeInTheDocument();
    });
  });

  describe('Tabela de Usuários - Tipo e Clube de Origem', () => {
    const mockUsersResponse = (users: object[]) => ({
      data: { users, total: users.length, limit: 20, offset: 0 },
    });

    const navigateToUsers = async (user: ReturnType<typeof userEvent.setup>) => {
      render(<AdminDashboard />);
      await screen.findByText(/Visão Geral/i);
      await user.click(screen.getByText(/^Usuários$/i));
    };

    it('exibe colunas Tipo e Clube de Origem na tabela de usuários', async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
      ...
      await navigateToUsers(user);
      expect(await screen.findByText('Usuário Teste')).toBeInTheDocument();
      expect(screen.getByText('Tipo')).toBeInTheDocument();
      expect(screen.getByText('Clube de Origem')).toBeInTheDocument();
    });

    it("exibe badge 'Anotador' e '—' no clube para usuário SCORER", async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
      ...
      await navigateToUsers(user);
      expect(await screen.findByText('João Anotador')).toBeInTheDocument();
      expect(screen.getByText('Anotador')).toBeInTheDocument();
    });

    it("exibe badge 'Atleta Avulso' e '—' no clube para usuário INDEPENDENT_ATHLETE", async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
      ...
      await navigateToUsers(user);
      expect(await screen.findByText('Maria Avulsa')).toBeInTheDocument();
      expect(screen.getByText('Atleta Avulso')).toBeInTheDocument();
    });

    it("exibe badge 'Espectador' para usuário SPECTATOR", async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
            mockUsersResponse([
              {
                id: 'u-spec',
                name: 'Carlos Espectador',
                email: 'spec@test.com',
                createdAt: '2026-01-01',
                platformRole: 'SPECTATOR',
                primaryClub: null,
                _count: { memberships: 0 },
              },
            ]),
      ...
      await navigateToUsers(user);
      expect(await screen.findByText('Carlos Espectador')).toBeInTheDocument();
      expect(screen.getByText('Espectador')).toBeInTheDocument();
    });

    it("exibe badge 'Membro' e nome do clube para usuário MEMBER com clube", async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
      ...
      await navigateToUsers(user);
      expect(await screen.findByText('Ana Membro')).toBeInTheDocument();
      expect(screen.getByText('Membro')).toBeInTheDocument();
      expect(screen.getByText('Clube Tênis SP')).toBeInTheDocument();
    });

    it("exibe badge 'Administrador' para usuário ADMIN", async () => {
      const user = userEvent.setup();
      mockHttpGet.mockImplementation((url: string) => {
        if (url.includes('/admin/users')) {
          return Promise.resolve(
