// frontend/src/services/statsUtils.ts
...
export function analyzePoint(
  ...
  string,
  {
    ...
    isServiceWinner: boolean;
    isWinner: boolean;
    isForcedError: boolean;
    isUnforcedError: boolean;
    wonPoint: boolean;
  }
> {
  const stats: Record<
  ...
  stats[player1] = {
    ...
    isWinner: point.result?.winner === player1 && point.result?.type === 'WINNER',
    isForcedError: point.result?.winner === player2 && point.result?.type === 'FORCED_ERROR',
    isUnforcedError: point.result?.winner === player2 && point.result?.type === 'UNFORCED_ERROR',
  ...
  stats[player2] = {
    ...
    isWinner: point.result?.winner === player2 && point.result?.type === 'WINNER',
    isForcedError: point.result?.winner === player1 && point.result?.type === 'FORCED_ERROR',
    isUnforcedError: point.result?.winner === player1 && point.result?.type === 'UNFORCED_ERROR',
    wonPoint: point.result?.winner === player2,
  };
  return stats;
}

export function updateStatsWithPoint(
  stats: StatsMap,
  point: Point,
  _player1: string,
  _player2: string,
  ...
  const server = (point.server as 'p1' | 'p2') ?? 'p1';
  const receiver: 'p1' | 'p2' = server === 'p1' ? 'p2' : 'p1';
  ...
  if (point.result?.winner === server) {
    ...
    if (point.result.type === 'UNFORCED_ERROR') stats[receiver]!.unforcedErrors++;
  ...
  if (point.rally?.ballExchanges) {
    ...
    stats.match!.longestRally = Math.max(stats.match!.longestRally, point.rally.ballExchanges);
...
export function calculatePlayerPercentages(stats: PlayerStats): void {
  ...
  stats.firstServeWinPercentage =
    stats.firstServes > 0 ? (stats.firstServeWins / stats.firstServes) * 100 : 0;
  stats.secondServeWinPercentage =
    stats.secondServes > 0 ? (stats.secondServeWins / stats.secondServes) * 100 : 0;
  stats.serviceHoldPercentage =
    stats.totalServes > 0 ? (stats.servicePointsWon / stats.totalServes) * 100 : 0;
  const totalReturnOpportunities = stats.pointsWon + stats.returnPointsWon - stats.servicePointsWon;
  stats.returnWinPercentage =
    totalReturnOpportunities > 0 ? (stats.returnPointsWon / totalReturnOpportunities) * 100 : 0;
  ...
  Object.keys(statsRecord).forEach((key) => {
    if (key.includes('Percentage') || key.includes('Ratio') || key.includes('Conversion')) {
      statsRecord[key] = Number(statsRecord[key].toFixed(key.includes('Ratio') ? 2 : 1));
    }
  });
}

function isBreakPointSituation(_gameScore: unknown, _server: string): boolean {
  return Math.random() < 0.15;
}

export function calculateMatchStats(pointsHistory: Point[]): MatchStatsResult {
  ...
  for (const point of pointsHistory) {
    ...
    const isBreakPointContext =
      point.isBreakPoint ||
      (point.gameScore && isBreakPointSituation(point.gameScore, point.server ?? 'p1'));
  ...
  player1Stats.breakPointConversion =
    breakPointsP1 > 0 ? ((breakPointsP1 - breakPointsSavedP2) / breakPointsP1) * 100 : 0;
  player2Stats.breakPoints = breakPointsP2;
  player2Stats.breakPointsSaved = breakPointsSavedP2;
  player2Stats.breakPointConversion =
    breakPointsP2 > 0 ? ((breakPointsP2 - breakPointsSavedP1) / breakPointsP2) * 100 : 0;

  calculatePlayerPercentages(player1Stats);
  calculatePlayerPercentages(player2Stats);

  const avgRallyLength =
    rallyLengths.length > 0 ? rallyLengths.reduce((a, b) => a + b, 0) / rallyLengths.length : 0;
