// frontend/src/services/statsUtils.js - JavaScript implementation for Node.js serverless

function createEmptyPlayerStats() {
  return {
    pointsWon: 0, totalServes: 0, firstServes: 0, secondServes: 0, firstServeWins: 0,
    secondServeWins: 0, aces: 0, doubleFaults: 0, serviceWinners: 0, servicePointsWon: 0,
    returnPointsWon: 0, winners: 0, unforcedErrors: 0, forcedErrors: 0, shortRallies: 0,
    longRallies: 0, breakPoints: 0, breakPointsSaved: 0, firstServePercentage: 0,
    firstServeWinPercentage: 0, secondServeWinPercentage: 0, serviceHoldPercentage: 0,
    breakPointConversion: 0, winnerToErrorRatio: 0, returnWinPercentage: 0, dominanceRatio: 0,
  };
}

function createEmptyMatchStats() {
  return { avgRallyLength: 0, longestRally: 0, shortestRally: 0, totalRallies: 0 };
}

function calculatePlayerPercentages(stats) {
  if (typeof stats.breakPointConversion !== 'number' || isNaN(stats.breakPointConversion)) {
    stats.breakPointConversion = 0;
  }
  stats.firstServePercentage = stats.totalServes > 0 ? (stats.firstServes / stats.totalServes) * 100 : 0;
  stats.firstServeWinPercentage = stats.firstServes > 0 ? (stats.firstServeWins / stats.firstServes) * 100 : 0;
  stats.secondServeWinPercentage = stats.secondServes > 0 ? (stats.secondServeWins / stats.secondServes) * 100 : 0;
  stats.serviceHoldPercentage = stats.totalServes > 0 ? (stats.servicePointsWon / stats.totalServes) * 100 : 0;
  const totalReturnOpportunities = stats.pointsWon + stats.returnPointsWon - stats.servicePointsWon;
  stats.returnWinPercentage = totalReturnOpportunities > 0 ? (stats.returnPointsWon / totalReturnOpportunities) * 100 : 0;
  stats.winnerToErrorRatio = stats.unforcedErrors + stats.forcedErrors > 0
    ? stats.winners / (stats.unforcedErrors + stats.forcedErrors)
    : stats.winners > 0 ? 999 : 0;
  stats.dominanceRatio = stats.unforcedErrors > 0
    ? (stats.winners + stats.forcedErrors) / stats.unforcedErrors
    : stats.winners + stats.forcedErrors > 0 ? 999 : 0;
  if (stats.breakPoints > 0) {
    stats.breakPointConversion = (stats.breakPointsSaved / stats.breakPoints) * 100;
  } else {
    stats.breakPointConversion = 0;
  }
  Object.keys(stats).forEach((key) => {
    if (key.includes('Percentage') || key.includes('Ratio') || key.includes('Conversion')) {
...
export function calculateMatchStats(pointsHistory) {
  ...
  const rallyLengths = [];
  let breakPointsP1 = 0, breakPointsP2 = 0, breakPointsSavedP1 = 0, breakPointsSavedP2 = 0;
  ...
  for (const point of pointsHistory) {
    ...
    if (point.serve) {
      ...
      switch (point.serve.type) {
        case 'ACE': currentPlayer.aces++; break;
        case 'DOUBLE_FAULT': currentPlayer.doubleFaults++; break;
        case 'SERVICE_WINNER': currentPlayer.serviceWinners++; break;
      }
  ...
  player1Stats.breakPointConversion = breakPointsP1 > 0 ? ((breakPointsP1 - breakPointsSavedP2) / breakPointsP1) * 100 : 0;
  player2Stats.breakPoints = breakPointsP2;
  player2Stats.breakPointsSaved = breakPointsSavedP2;
  player2Stats.breakPointConversion = breakPointsP2 > 0 ? ((breakPointsP2 - breakPointsSavedP1) / breakPointsP2) * 100 : 0;

  calculatePlayerPercentages(player1Stats);
  calculatePlayerPercentages(player2Stats);

  const avgRallyLength = rallyLengths.length > 0 ? rallyLengths.reduce((a, b) => a + b, 0) / rallyLengths.length : 0;
