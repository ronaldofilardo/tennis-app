...
const JWT_SECRET = process.env.JWT_SECRET || 'racket-app-dev-secret-change-in-production';
 ...
 * @param {{ userId: string, email: string, clubId?: string, role?: string, platformRole?: string, planType?: string, subscriptionStatus?: string }} payload
 * @returns {string}
 */
...
export async function getClubMembers(clubId, excludeUserId = null) {
  return prisma.clubMembership.findMany({
    where: {
      clubId,
      role: { notIn: ['ADMIN', 'SPECTATOR'] }, // ADMIN e SPECTATOR são independentes de clube
