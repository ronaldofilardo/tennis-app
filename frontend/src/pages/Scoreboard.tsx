...
const Scoreboard: React.FC<ScoreboardProps> = ({
  ...
  useEffect(() => {
    const updateMatchStatus = async () => {
      if (match.id && !initialState) {
        // Só atualizar se for nova partida
        try {
  ...
  const handleAddPoint = async (player: "PLAYER_1" | "PLAYER_2") => {
    ...
    if (newState.isFinished) {
      ...
      setTimeout(() => {
        // AREA 4: Toast substitui alert() nativo (themeable para White Label)
        toast.success(`Vencedor: ${winner}`, "Partida Finalizada!");
        if (onMatchFinished)
          onMatchFinished(
            match.id,
            score,
  ...
  return (
    <div className="scoreboard" role="region" aria-label="scoreboard">
      <div className="scoreboard-header">
        <h3>
          {match.sportType} -{" "}
          {TennisConfigFactory.getFormatDisplayName(
            (match.format || "BEST_OF_3") as TennisFormat,
          )}
        </h3>
        <button onClick={onEndMatch} className="end-match-button">
          Encerrar Partida
        </button>
      </div>
      ...
      <div className="score-display">
        <div className="team">
          ...
          <div className="game-score">
            {matchState.currentGame.points.PLAYER_1}
          </div>
        </div>
        ...
        <div className="team">
          ...
          <div className="game-score">
            {matchState.currentGame.points.PLAYER_2}
          </div>
        </div>
      </div>

      <div
        className={`match-status ${matchState.isFinished ? "finished" : "in-progress"}`}
      >
        {matchState.isFinished ? (
          <div className="final-result">
            <span className="status-label">RESULTADO FINAL:</span>
            <span className="winner-highlight">
              {matchState.winner === "PLAYER_1"
                ? match.players?.p1 || "Jogador A"
                : match.players?.p2 || "Jogador B"}{" "}
              VENCEU!
            </span>
          ...
          <div className="live-status">
            ...
            <span className="status-detail">
              ...
              Games: {matchState.currentSetState.games.PLAYER_1}-
              {matchState.currentSetState.games.PLAYER_2} | Pontos:{" "}
              {matchState.currentGame.points.PLAYER_1}-
              {matchState.currentGame.points.PLAYER_2}
            </span>
          </div>
      ...
      {matchState.currentGame.isTiebreak && (
        <div className="tiebreak-info" data-testid="tiebreak-info">
          <span>
            🏆{" "}
            {matchState.currentGame.isMatchTiebreak
              ? "MATCH TIEBREAK"
              : "TIEBREAK"}
          </span>
        </div>
          ...
          <table className="partials-table">
            ...
            <tbody>
              {matchState.completedSets.map((s) => (
                <tr
                  key={s.setNumber}
                  className={s.winner === "PLAYER_1" ? "row-p1" : "row-p2"}
                >
                  <td>{s.setNumber}</td>
