    ...
    for (let i = 0; i < 3; i++) {
      match.addPoint(PLAYER_1, {
        ...
        result: {
          winner: PLAYER_1,
          type:
            i === 1 ? "WINNER" : i === 2 ? "UNFORCED_ERROR" : "FORCED_ERROR",
          finalShot: "FOREHAND",
        },
...
describe("TennisScoring - Cobertura de métodos e edge cases", () => {
  ...
  test("loadState: restaura estado e lida com formato diferente", () => {
    ...
    const fakeState = {
      ...
      config: {
        ...state.config,
        format: "FAST4" as import("./types").TennisFormat,
      },
    };
  ...
  test("shouldPlayMatchTiebreak e isDecidingSet", () => {
    ...
    for (let g = 0; g < 6; g++)
      for (let p = 0; p < 4; p++) match.addPoint(PLAYER_1); // 1º set
    for (let g = 0; g < 6; g++)
      for (let p = 0; p < 4; p++) match.addPoint(PLAYER_2); // 2º set
...
describe("TennisScoring - syncState com autenticação", () => {
  const PLAYER_1: Player = "PLAYER_1";
  ...
  test("syncState envia Authorization header quando token existe no localStorage", async () => {
    ...
    const match = new TennisScoring(PLAYER_1, "BEST_OF_3");
    match.enableSync("match-123");
    await match.syncState();
  ...
  test("syncState NÃO envia Authorization quando localStorage sem token", async () => {
    vi.stubGlobal("localStorage", {
  ...
  test("syncState envia getState() com pointsHistory no body", async () => {
    vi.stubGlobal("localStorage", {
      getItem: vi.fn().mockReturnValue("tok"),
      setItem: vi.fn(),
      removeItem: vi.fn(),
    });

    const match = new TennisScoring(PLAYER_1, "BEST_OF_3");
    match.enableSync("match-789");
    match.addPoint(PLAYER_1);
    await match.syncState();
    ...
    expect(body.matchState).toHaveProperty("pointsHistory");
    expect(Array.isArray(body.matchState.pointsHistory)).toBe(true);
  });
});
