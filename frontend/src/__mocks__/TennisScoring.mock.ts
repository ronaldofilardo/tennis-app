...
const buildSnapshot = (
  state: MockState & { pointP1: string; setsP1: number; setsP2: number },
) => ({
  ...
  currentSetState: {
    games: { PLAYER_1: state.gamesP1, PLAYER_2: state.gamesP2 },
  },
  currentGame: {
    points: { PLAYER_1: state.pointP1, PLAYER_2: "0" },
    isTiebreak: false,
  },
});
...
const createAddPointSyncMock = () =>
  vi.fn((player?: unknown) => {
    if (!mockState.finished) {
      const idx = pointOrder.indexOf(mockState.pointP1);
      if (idx < pointOrder.length - 1) {
        mockState.pointP1 = pointOrder[idx + 1];
...
export function __resetMockTennisScoring(
  overrides: Partial<typeof mockTennisScoring> = {},
) {
  mockState = {
    gamesP1: 0,
    gamesP2: 0,
