...
function validateRow(row: AthleteRow): AthleteRow {
  ...
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email.trim())) errors.push('E-mail inválido');
...
const BulkAthleteImport: React.FC<BulkAthleteImportProps> = ({ clubId, onComplete, onCancel }) => {
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (raw.length < 2) {
            toast.error('Planilha vazia ou sem dados. A primeira linha deve conter os cabeçalhos.');
            return;
          }
          ...
          for (let r = 1; r < raw.length; r++) {
            ...
            Object.entries(headerMap).forEach(([colIdx, field]) => {
              ...
              if (field === 'birthDate') {
                ...
                athlete[field] = cellValue != null ? String(cellValue).trim() : undefined;
              }
            });
          ...
          if (rows.length > 500) {
            toast.error(`Máximo de 500 registros por importação. A planilha tem ${rows.length}.`);
            return;
          }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const response = await httpClient.post<ImportResult>(`/clubs/${clubId}/members/import`, {
        athletes: payload,
      });
      ...
      if (response.data.created > 0) {
        toast.success(`${response.data.created} atleta(s) importado(s) com sucesso!`);
      }
    ...
    <div className="bulk-import-container">
      <div className="bulk-import-header">
        ...
        <p className="bulk-import-subtitle">
          Envie uma planilha XLSX com os dados dos atletas. Cada atleta receberá uma conta com senha
          temporária (6 primeiros dígitos do CPF ou "123456").
        </p>
      </div>
      ...
      {step === 'upload' && (
        <div className="bulk-import-upload">
          <div className="bulk-import-actions-top">
            <button type="button" className="bulk-import-btn-template" onClick={downloadTemplate}>
              📋 Baixar Template
            </button>
          </div>
          ...
          >
            ...
            <span className="dropzone-hint">Máximo 500 registros por importação</span>
            <input
              ref={fileInputRef}
              type="file"
              accept=".xlsx,.xls"
          ...
          <div className="bulk-import-fields-info">
            ...
            <div className="fields-docs-preview">
              ...
              <span className="field-tag disabled">🏥 Atestado Médico (em breve)</span>
            </div>
          </div>

          {onCancel && (
            <button type="button" className="bulk-import-btn-cancel" onClick={onCancel}>
              Cancelar
            </button>
          )}
        </div>
        ...
        <div className="bulk-import-preview">
          <div className="preview-summary">
            ...
            <span className="preview-stats">
              ...
              {invalidCount > 0 && (
                <span className="stat-invalid">❌ {invalidCount} com erros</span>
              )}
          ...
          {unmappedCols.length > 0 && (
            <div className="preview-warning">
              ⚠️ Colunas ignoradas (não reconhecidas): <strong>{unmappedCols.join(', ')}</strong>
            </div>
          )}
              ...
              <tbody>
                {athletes.map((a, idx) => (
                  <tr key={idx} className={a._valid ? '' : 'row-invalid'}>
                    ...
                    <td>
                      {a.gender === 'MALE' ? 'M' : a.gender === 'FEMALE' ? 'F' : a.gender || '—'}
                    </td>
                    <td>
                      {a.birthDate
                        ? new Date(a.birthDate + 'T00:00:00').toLocaleDateString('pt-BR')
                        : '—'}
                    </td>
                    <td>{a.category || '—'}</td>
                    <td>{a.entity || '—'}</td>
          ...
          <div className="preview-actions">
            <button type="button" className="bulk-import-btn-cancel" onClick={handleReset}>
              ← Voltar
            </button>
            <button
              type="button"
      ...
      {step === 'result' && result && (
        <div className="bulk-import-result">
          ...
          <div className="result-actions">
            ...
            <button type="button" className="bulk-import-btn-template" onClick={handleReset}>
              Nova Importação
            </button>
          </div>
        </div>
